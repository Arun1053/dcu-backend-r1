...
function KPICard({ label, actual, optimum, unit, highlight, onTrend }) {
  ...
  return (
    <div
      ...
      title={onTrend ? "Click to view trend" : ""}
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.textMid, letterSpacing: 0.2 }}>{label}</div>
        {onTrend && <div style={{ fontSize: 14, color: C.accent }} aria-label="trend">📈</div>}
      </div>

      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: valColor }}>{actual ?? "—"}</div>
        {unit && <div style={{ fontSize: 11, fontWeight: 700, color: C.muted }}>{unit}</div>}
      </div>

      {optimum != null && (
        <div style={{ fontSize: 10, color: C.muted }}>
          Optimum: <span style={{ fontWeight: 700 }}>{optimum}</span>
        </div>
      )}
    </div>
  );
...
function computeDomainFromTrend(trend = [], cfg) {
  if (!cfg) return [0, 1];

  // Step / categorical charts (0/1)
  if (cfg.isStep) {
    return [0, 1];
  }

  const keys = [cfg.metricKey, cfg.upperKey, cfg.lowerKey].filter(Boolean);
  ...
  // include optimum if provided
  if (cfg.optimumValue != null) {
    const ov = toNum(cfg.optimumValue);
    if (ov != null) {
      if (ov < min) min = ov;
      if (ov > max) max = ov;
    }
  ...
  // fallback if no numeric values exist
  ...
  const range = max - min;
  const pad = range === 0 ? Math.max(1, Math.abs(max) * 0.05) : range * 0.08;

  const yMin = min - pad;
  const yMax = max + pad;

  // if still flat, widen slightly
  if (yMin === yMax) return [yMin - 1, yMax + 1];

  return [yMin, yMax];
}

function computeTicks(domain, cfg) {
  if (cfg?.isStep) return cfg.yTicks || [0, 1];
  return undefined; // let recharts auto-pick
}
...
export default function App() {
  ...
  const [popupChart, setPopupChart] = useState(null); // chart config object


  const fetchData = async () => {
    try {
      const res = await axios.get("http://localhost:8000/run-model");
      // const res = await axios.get("https://dcu-backend.onrender.com/run-model");
      setData(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  // ── Trend popup configuration for ALL KPI tiles ──
  // Each label maps to a chart config. Ensure backend returns these keys inside data.trend[].
  const popupConfigByLabel = {
    "HGI Prediction": { title: "HGI Prediction – Trend", metricKey: "hgi", unit: "%", upperKey: "upper", lowerKey: "lower", optimumValue: Number(optHGI), yPadding: 0.12, hardDomain: null},
    "Target Range": { title: "Target Range (Upper/Lower) – Trend", metricKey: "hgi", unit: "%", upperKey: "upper", lowerKey: "lower", optimumValue: Number(optHGI) },
    "Online Hours": { title: "Online Hours – Trend", metricKey: "houronline", unit: "hrs", optimumValue: 3.0 },
    "Furnace Charge": { title: "Furnace Charge – Trend", metricKey: "furnacecharge", unit: "t/d", optimumValue: Number(optFurnace)},

    "Upper Bound": { title: "Upper Bound – Trend", metricKey: "upper", unit: "%" },
    "Lower Bound": { title: "Lower Bound – Trend", metricKey: "lower", unit: "%" },
    "Prediction Hi": { title: "Prediction Hi – Trend", metricKey: "upper", unit: "%" },

    "Heavy Yield": { title: "Heavy Yield – Trend", metricKey: "yield_heavy", unit: "%" },
    "Medium Yield": { title: "Medium Yield – Trend", metricKey: "yield_medium", unit: "%" },
    "Light Yield": { title: "Light Yield – Trend", metricKey: "yield_light", unit: "%" },

    // Categorical tiles shown as step charts with 0/1 mapping
    "Drum 1": { title: "Drum 1 – Trend (0/1)", metricKey: "drum1_bin", unit: "0/1", isStep: true, yTicks: [0, 1] },
    "Drum 2": { title: "Drum 2 – Trend (0/1)", metricKey: "drum2_bin", unit: "0/1", isStep: true, yTicks: [0, 1] },
    "Drum 1 Status": { title: "Drum 1 Status – Trend (0/1)", metricKey: "drum1_bin", unit: "0/1", isStep: true, yTicks: [0, 1] },
    "Drum 2 Status": { title: "Drum 2 Status – Trend (0/1)", metricKey: "drum2_bin", unit: "0/1", isStep: true, yTicks: [0, 1] },
    "Flag 1": { title: "Flag 1 – Trend (0/1)", metricKey: "flag1", unit: "0/1", isStep: true, yTicks: [0, 1] },
    "Flag 2": { title: "Flag 2 – Trend (0/1)", metricKey: "flag2", unit: "0/1", isStep: true, yTicks: [0, 1] },
  };

  const openPopup = (label) => {
    const cfg = popupConfigByLabel[label];
    if (!cfg) return;
    setPopupChart(cfg);
  };

  // Trend safety guard for popup (prevents blank popup when trend is missing)
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              ...
              <div style={{ background: C.white, borderRadius: 10, padding: 14, border: `1px solid ${C.border}` }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <KPICard label="Furnace Charge" actual={d.furnacecharge} unit="t/d" onTrend={() => openPopup("Furnace Charge")} />
                  <KPICard label="HGI Prediction" actual={d.prediction} unit="%" highlight={alarmColor === "red"} onTrend={() => openPopup("HGI Prediction")} />
                  <KPICard label="Target Range" actual={`${d.lower} – ${d.upper}`} unit=""  onTrend={() => openPopup("Target Range")} />
                  <KPICard label="Online Hours" actual={d.houronline?.toFixed(2)} unit="hrs" onTrend={() => openPopup("Online Hours")} />
                  <KPICard label="Heavy Yield" actual={`${d.yields?.heavy ?? "—"}%`} unit=""  onTrend={() => openPopup("Heavy Yield")} />
                  <KPICard label="Light Yield" actual={`${d.yields?.light ?? "—"}%`} unit=""  onTrend={() => openPopup("Light Yield")} />
                </div>
              </div>
              ...
              <div style={{ background: C.white, borderRadius: 10, padding: 14, border: `1px solid ${C.border}` }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <KPICard
                   ...
                   onTrend={() => openPopup("Drum 1")} />
                  <KPICard
                    label="Drum 2"
                    actual={d.drum_status?.drum2 ?? "—"}
                    highlight={d.drum_status?.drum2 === "Offline"}
                   onTrend={() => openPopup("Drum 2")} />
                  <KPICard label="Flag 1" actual={d.flags?.flag1} optimum="0"  onTrend={() => openPopup("Flag 1")} />
                  <KPICard label="Flag 2" actual={d.flags?.flag2} optimum="0"  onTrend={() => openPopup("Flag 2")} />
                  <KPICard label="Medium Yield" actual={`${d.yields?.medium ?? "—"}%`} unit=""  onTrend={() => openPopup("Medium Yield")} />
                  <KPICard label="Prediction Hi" actual={d.upper} unit=""  onTrend={() => openPopup("Prediction Hi")} />
                </div>
              </div>
              ...
              <div style={{ background: C.white, borderRadius: 10, padding: 14, border: `1px solid ${C.border}` }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <KPICard label="Drum 1 Status" actual={d.drum_status?.drum1}  onTrend={() => openPopup("Drum 1 Status")} />
                  <KPICard label="Drum 2 Status" actual={d.drum_status?.drum2}  onTrend={() => openPopup("Drum 2 Status")} />
                  <KPICard label="Flag 1" actual={d.flags?.flag1} optimum="1"  onTrend={() => openPopup("Flag 1")} />
                  <KPICard label="Flag 2" actual={d.flags?.flag2} optimum="2"  onTrend={() => openPopup("Flag 2")} />
                  <KPICard label="Lower Bound" actual={d.lower} unit=""  onTrend={() => openPopup("Lower Bound")} />
                  <KPICard label="Upper Bound" actual={d.upper} unit="" highlight  onTrend={() => openPopup("Upper Bound")} />
                </div>
              </div>
            </div>
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: C.textDark }}>
                Current: {(() => {
                  ...
                  const v = last[popupChart.metricKey];
                  if (v == null || v === "") return "—";
                  if (popupChart.isStep) return String(v);
                  return typeof v === "number" ? v.toFixed(2) : String(v);
                })()} {popupChart.unit || ""}
              </div>

              <div style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 11, color: C.muted }}>
                {popupChart.upperKey && <span>Upper: <b style={{ color: C.red }}>line</b></span>}
                {popupChart.lowerKey && <span>Lower: <b style={{ color: C.orange }}>line</b></span>}
                {popupChart.optimumValue != null && <span>Optimum: <b style={{ color: C.teal }}>{popupChart.optimumValue}</b></span>}
              </div>
            </div>
            ...
            <div style={{ width: "100%", height: 360 }}>
              ...
              {(() => {
                const yDomain = computeDomainFromTrend(trendRows, popupChart);
                const yTicks = computeTicks(yDomain, popupChart)
              return(
              <ResponsiveContainer width="100%" height="100%">
                {hasTrend ? (
                <LineChart data={data.trend} margin={{ top: 10, right: 18, left: 6, bottom: 8 }}>
                  ...
                  <Line
                    type={popupChart.isStep ? "stepAfter" : "monotone"}
                    dataKey={popupChart.metricKey}
                    name={popupChart.title.replace(" – Trend", "")}
                    stroke={C.accent}
                    strokeWidth={3}
                    dot={false}
                  />
                  ...
                  {popupChart.upperKey && (
                    <Line type="monotone" dataKey={popupChart.upperKey} name="Upper Bound" stroke={C.red} strokeDasharray="6 4" dot={false} />
                  )}
                  {popupChart.lowerKey && (
                    <Line type="monotone" dataKey={popupChart.lowerKey} name="Lower Bound" stroke={C.orange} strokeDasharray="6 4" dot={false} />
                  )}

                  {popupChart.optimumValue != null && (
                    <ReferenceLine y={popupChart.optimumValue} stroke={C.teal} strokeDasharray="4 4" label={{ value: "Optimum", fill: C.teal, fontSize: 10 }} />
                  )}
                </LineChart>
                ) : (
                
                <div style={{ padding: 20, color: C.muted, fontWeight: 600 }}>
                      No trend data available for this KPI yet.
                    </div>
                )}
